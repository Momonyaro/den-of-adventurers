extends BaseState;
class_name OnRecruitState;

var assigned_activity: bool = false;

func _init():
	_state_ref = StateReference.ON_RECRUIT;
...
func evaluate(agent: Agent, _adv_state: Adventurer.Status, has_destination: bool, party: Party) -> bool:
	if _adv_state == Adventurer.Status.RECRUIT:
		return true;
	return false;
...
func update(delta: float, agent: Node, camera: Node):
	...
	if activity_manager != null && !assigned_activity:
		var activity_instance = activity_manager.try_get_activity("recruit_wait", agent);
